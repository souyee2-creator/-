...
const MenuItem: React.FC<MenuItemProps> = ({ symbol, label, desc, onClick }) => (
  ...
  >
    <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
      <div style={{ width: 44, height: 44, border: '1px solid #111', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        <span style={{ fontSize: 19, color: '#111', fontFamily: 'Georgia, serif', lineHeight: 1 }}>{symbol}</span>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
        <span style={{ fontSize: 16, fontWeight: 700, color: '#111', fontFamily: 'Georgia, serif' }}>{label}</span>
        <span style={{ fontSize: 8, letterSpacing: '0.26em', textTransform: 'uppercase', color: '#bbb', fontFamily: 'Georgia, serif' }}>{desc}</span>
      </div>
    </div>
    <span style={{ fontSize: 16, color: '#ccc', fontFamily: 'Georgia, serif' }}>›</span>
  </motion.div>
);
...
export const CorePage: React.FC<CorePageProps> = ({ favorites, onRemoveFavorite }) => {
  ...
  return (
    <div style={{ position: 'relative', flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, height: '100%' }}>
      <div style={{ flex: 1, overflowY: 'auto', background: '#ffffff', fontFamily: 'Georgia, serif' }}>
        <div style={{ maxWidth: 520, margin: '0 auto', padding: 'calc(env(safe-area-inset-top) + 32px) 28px 28px' }}>
          ...
          <div style={{ marginBottom: 28 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '10px 0 14px' }}>
              ...
              <div
                ...
                style={{ width: 80, height: 80, border: '2px solid #111', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#111', flexShrink: 0, cursor: 'pointer', overflow: 'hidden', position: 'relative' }}
              >
                {profile.avatar
                  ? <img src={profile.avatar} alt="avatar" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  : <span style={{ fontSize: 32, color: '#ffffff', fontFamily: 'Georgia, serif' }}>{displayInitial}</span>
                }
                <div
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                {editingName ? (
                  <input
                    ...
                    placeholder="输入昵称"
                    style={{ fontFamily: 'Georgia, serif', fontSize: 32, fontWeight: 700, letterSpacing: '-0.02em', color: '#111', background: 'transparent', border: 'none', borderBottom: '1px solid #111', outline: 'none', width: '100%' }}
                  />
                ) : (
                  <h2
                    onClick={() => setEditingName(true)}
                    title="点击修改昵称"
                    style={{ margin: 0, fontSize: 32, fontWeight: 700, letterSpacing: '-0.02em', color: profile.name ? '#111' : '#ccc', fontFamily: 'Georgia, serif', lineHeight: 1.1, cursor: 'text' }}
                  >
      ...
      <AnimatePresence>
        {showMasks && (
          <motion.div
            ...
            style={{ position: 'fixed', inset: 0, zIndex: 100, overflow: 'hidden', background: '#ffffff' }}
          >
